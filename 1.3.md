...
3. **コスト関数の定義**
    コスト関数$V(\mathbf{x}_{k})$は，現在の状態$\mathbf{x}_{k}$から**最適な**制御入力を取った時に期待される
    最小コスト
    $$
...
コスト関数： $\scriptsize V(\mathbf{x}_{k}) = \mathrm{min_{\mathbf{u}_{k}}}\sum_{k=k_{s}}^\infty (\mathbf{x}^\mathrm{T}_{k}Q\mathbf{x}_{k} + \mathbf{u}^\mathrm{T}_{k}R\mathbf{u}_{k})$
...
4. **ベルマン最適方程式の導出**
    **コスト関数の再帰的表現**
    ...
    つまり，コスト関数$V(\mathbf{x}_{k})$は，次のように表すことができる．
    $$
...
4. **ベルマン最適方程式の導出**
    **コスト関数の二次形式仮定**
    $\small V(\mathbf{x}_{k+1})$を簡単にするため，コスト関数が二次形式であると仮定し，
    次のように表す．
...
**コスト関数の二次形式仮定**： $\small V(\mathbf{x}_{k+1}) = \mathbf{x}^\mathrm{T}_{k+1}P\mathbf{x}_{k+1}$ 
